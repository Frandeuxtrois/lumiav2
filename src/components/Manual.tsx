  ...
  <div className="fixed inset-0 bg-ink/40 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
    <div className="bg-surface rounded-awd shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
      ...
      <div className="flex items-center justify-between px-6 py-5 border-b border-line shrink-0">
        ...
        <button onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-awd hover:bg-elevated text-muted hover:text-muted transition-colors">
          <X size={18} />
        </button>
      </div>
      ...
      <div className="overflow-y-auto p-6 space-y-4">
        ...
        <Section icon={<Users size={16} />} title="¿Qué ve y vive tu cliente?">
          <p className="text-muted mb-2">Cuando un cliente entra a tu página de turnos, esto es lo que pasa paso a paso:</p>
        ...
        <Section icon={<CalendarDays size={16} />} title="Crear horarios disponibles">
          <p className="text-muted mb-3">Hacé clic en el botón <strong>"Nuevo Horario"</strong> (arriba a la derecha). Podés elegir entre tres formas de cargar:</p>
          <div className="space-y-2">
            <div className="bg-elevated rounded-awd p-3">
              <p className="font-semibold mb-1">Individual — para casos puntuales</p>
              <p className="text-muted">Elegís una fecha y un horario específico. Te crea ese único turno disponible.</p>
            </div>
            <div className="bg-elevated rounded-awd p-3">
              <p className="font-semibold mb-1">Rango — para un día completo</p>
              <p className="text-muted">Elegís una fecha, el horario de inicio, el horario de cierre y cuánto dura cada turno (30, 45, 60 o 90 minutos). El sistema crea todos los turnos del día solo. Antes de confirmar te muestra cuántos va a crear.</p>
            </div>
            <div className="bg-elevated rounded-awd p-3">
              <p className="font-semibold mb-1">Período — para varias semanas de una vez</p>
              <p className="text-muted">Elegís qué días de la semana trabajás, el rango de fechas (por ejemplo, todo el mes de junio) y el horario. El sistema carga todos los turnos de golpe.</p>
            </div>
          </div>
          <p className="text-muted text-xs mt-2">Si intentás cargar un horario que ya existe, el sistema lo saltea sin que tengas que hacer nada.</p>
        </Section>
        ...
        <Section icon={<CalendarRange size={16} />} title="Dos formas de ver tu agenda">
          <div className="space-y-2">
            <div className="bg-elevated rounded-awd p-3">
              ...
              <p className="text-muted">Ves todos los turnos de un día específico en una lista. Usá el selector de fecha para ir al día que quieras.</p>
            </div>
            <div className="bg-elevated rounded-awd p-3">
              <p className="font-semibold mb-1">Vista Semana</p>
              <p className="text-muted">Ves los 7 días de la semana de un vistazo. Ideal para planificar o ver cómo quedó cargada la semana. Navegás con las flechas para ir a la semana anterior o siguiente.</p>
            </div>
          </div>
        </Section>
        ...
        <Section icon={<CheckCircle2 size={16} />} title="Estados de los turnos — ¿qué significa cada color?">
          <div className="space-y-2">
            <Tag color="bg-accent/10 text-accent" label="Disponible" desc="El horario está libre. Los clientes lo pueden ver y reservar." />
            <Tag color="bg-info/10 text-info" label="Reservado" desc="Un cliente tomó ese turno y el sistema ya le envió el email de confirmación." />
            <Tag color="bg-elevated text-muted" label="Completado" desc="El turno se realizó. Lo marcás vos para llevar registro." />
            <Tag color="bg-danger/10 text-danger" label="Cancelado" desc="El turno fue cancelado, ya sea por el cliente o por vos." />
          </div>
          <div className="mt-3 space-y-1.5">
            <p className="text-xs font-semibold uppercase tracking-wider text-muted mb-1">Qué podés hacer con cada turno:</p>
            <div className="flex items-center gap-2"><CheckCircle2 size={14} className="text-accent shrink-0" /><span>Marcar como completado (solo aparece en turnos reservados)</span></div>
            <div className="flex items-center gap-2"><Trash2 size={14} className="text-danger shrink-0" /><span>Eliminar el turno. Si había un cliente, le llega un email avisándole automáticamente.</span></div>
          </div>
        </Section>
        ...
        <Section icon={<XCircle size={16} />} title="Cancelación de turnos">
          <div className="space-y-3">
            <div className="bg-elevated rounded-awd p-3">
              ...
              <p className="text-muted">En el email que recibe el cliente hay un botón rojo "Cancelar turno". Si lo usa con más de 48 horas de anticipación, el turno queda libre automáticamente y <strong>vos recibís un email avisándote</strong>. Si intenta cancelar con menos de 48 horas, el sistema se lo impide.</p>
            </div>
            <div className="bg-elevated rounded-awd p-3">
              <p className="font-semibold mb-1">Si cancelás vos desde el panel</p>
              <p className="text-muted">Eliminás el turno reservado desde la tabla. Antes de confirmar, el sistema te avisa. Si confirmás, <strong>el cliente recibe un email automáticamente</strong> notificándole que su turno fue cancelado.</p>
            </div>
          </div>
        </Section>

        <Section icon={<Bell size={16} />} title="Recordatorios automáticos — no tenés que hacer nada">
          <p className="text-muted mb-3">El sistema recuerda a los clientes sus turnos de forma automática. Vos no intervenís para nada:</p>
          <div className="space-y-2">
            <div className="flex items-center gap-3 bg-elevated rounded-awd p-3">
              <span className="text-2xl">⏰</span>
              <div>
                <p className="font-semibold">El día anterior</p>
                <p className="text-muted">El cliente recibe un email recordándole el turno del día siguiente, con la opción de cancelar si lo necesita.</p>
              </div>
            </div>
            <div className="flex items-center gap-3 bg-elevated rounded-awd p-3">
              <span className="text-2xl">⏰</span>
              <div>
                ...
                <p className="text-muted">Un segundo recordatorio el mismo día para que no se olvide.</p>
              </div>
            </div>
          </div>
        </Section>
        ...
        <Section icon={<Mail size={16} />} title="Conectar tu Gmail para enviar emails">
          <p className="text-muted mb-3">El sistema necesita conectarse a tu Gmail para poder enviar los emails de confirmación y recordatorios. Lo hacés una sola vez:</p>
          ...
          <div className="bg-warning/10 border border-warning/25 rounded-awd p-3 mt-2 text-warning text-xs">
        ...
        <Section icon={<Image size={16} />} title="Tu foto de perfil">
          <p className="text-muted mb-2">Tu foto aparece en la página donde los clientes reservan, para que sepan con quién están hablando.</p>
        ...
        <Section icon={<Lock size={16} />} title="Cambiar tu contraseña de acceso">
          <p className="text-muted mb-2">Si querés cambiar la contraseña con la que entrás al panel:</p>
